...
void ws2812_indicate_layer(void) {
    ...
    queue_blink(hex_to_rgb(CONFIG_WS2812_WIDGET_LAYER_COLOR_MANUAL),
                ...
                CONFIG_WS2812_WIDGET_INTERVAL_MS,
...
void ws2812_indicate_battery(void) {
    ...
    if (battery_level == 0) {
        LOG_WRN("WS2812 battery indication skipped: battery level unavailable");
        return;
    }

    queue_blink(get_battery_color(battery_level),
                CONFIG_WS2812_WIDGET_BATTERY_BLINK_MS,
                CONFIG_WS2812_WIDGET_INTERVAL_MS,
                CONFIG_WS2812_WIDGET_BATTERY_BLINK_REPEAT);

    LOG_INF("WS2812 battery indication: level=%d", battery_level);
}
